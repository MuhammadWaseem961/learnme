...
@section('title','Client | Dashboard') {{-- head start --}} 
                            ...
                            <div class="col-md-10 p-0 cl-6A6A6A">
                                ...
                                <div class="mt-3 row align-items-center bg-F2F5FA box_shadow2">
                                    ...
                                    <div class="col-md-8 text-right">
                                        ...
                                        <img src="{{ asset('assets/frontend/images/video-call-icon.png') }}" onclick="makeCall(this)" class="d-none img-fluid h-40 video-chat video-chat-img-{{ $appointment->id }}" data-id="{{ $appointment->id }}" id="video-chat" data-booking-id="{{ $appointment->id }}" data-sender="{{ $appointment->user->id }}" data-reciever="{{ $appointment->specialist->id }}" data-caller="{{$appointment->specialist->username}}">
                                    </div>
                                </div>
                                @endforeach
            ...
            <div class="col-md-12 px-5 padding_dashboard borderRadius-10px box_shadow1 p-0">
                ...
                <form id="create-form" method="post" enctype="multipart/form-data">
                    ...
                    <div class=" pl-0 ml-auto text-end pr-0 my-3">
                        <button type="button" onclick="createUpdateResource('{{ route('servicerequests.store') }}','','post','create-form')" class="btn btn-outline-success my-2 d-flex justify-content-end my-sm-0 cl-ffffff bg-3ac574 pl-2 pr-2 login_button ml-auto">Create Post</button>
                    </div>
                </form>
                            ...
                            @if($service_requests->count() >0)
                                @foreach($service_requests as $i=>$request)
                                    ...
                                    <tr id="target_" class="border-bottom">
                                        ...
                                        <td><span class="badge badge-sm {{$request->status == "1"?'badge-success':'badge-danger'}}">{{$request->status == "1"?'Approved':'Pending'}}</span></td>
                                        <td>
                                                                    ...
                                                                    <div class="col-md-12 mt-3 borderRadius-10px box_shadow1 pb-5">
                                                                        ...
                                                                        @foreach ($request->bids as $service)
                                                                            <div class="d-flex flex-directionSmall mt-4 justify-content-between pr-5" >
                                                                                ...
                                                                                <div class="col-md-1">
                                                                                    @if($service->work_status=="Un-Complete")
                                            ...
                                            <button class="btn btn-danger btn-sm ml-1" title="Delete {{$request->title}} Post" onclick="deleteResource('{{ route('servicerequests.destroy',$request->id) }}','')">Delete</button>
                                            @if($request->bids->count() > 0)
