...
@if(count($categories) > 0)
    <section class=" main_padding pt-5">
        <div>
            <ul class="listStyle-none p-0 d-flex robotoRegular f-16 ul_main_tabs m-0 d-flex  align-items-center flex-directionSmall cl-3b3b3b3"
            id="main_navbar">
                @foreach ($categories->take(8) as $category)
                    <li class="pr-3"> <a href="{{ route('category.events',['name'=>str_replace(' ','-',$category->title),'id'=>$category->id])}}" class="cl-3b3b3b3">{{ ucwords($category->title) }}</a></li>
                @endforeach
                        ...
                        <div class="btn-group" style='position: relative;'>
                            ...
                            <ul class="dropdown-menu dropdown-menu-1">
                                @foreach ($categories->skip(8) as $category)
                                    <li class="pl-3"> <a href="{{ route('category.events',['name'=>str_replace(' ','-',$category->title),'id'=>$category->id])}}" class="cl-3b3b3b3">{{ ucwords($category->title) }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </li>
