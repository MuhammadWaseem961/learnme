@extends('layouts.admin.admin') @section('title','Event Categories') {{-- head start --}} @section('extra-css')
...
<p class="pl-3 f-21 cl-000000">Event Categories</p>
@include('common.messages')
<button title="Click to Add Service" data-toggle="modal" data-target="#addCatModal"
        class="btn btn-sm bg-3AC574 text-white m-2 mr-3" style="float: right;">Add Event Category</button>
            ...
            @foreach($categories as $key => $category)
                <tr id="target_{{ $category->id }}">
                    ...
                    <td style="min-width: 135px !important;">
                        ...
                        <button title="Click to Delete Category" type="button" class="btn btn-danger btn-sm"  onclick="deleteResource('{{route('eventCategories.destroy',$category->id)}}','{{route('eventCategories.index')}}')"><i class="fa fa-trash"></i> </button>
                    </td>
                </tr>
            @endforeach
        </tbody>
                    ...
                    <form id="add-form" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input id="name" type="text" class="form-control text-capitalize" name="title" value="{{ old('name') }}" autocomplete="name" placeholder="Enter Title" required />
                            </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-md bg-3AC574 text-white" onclick="addUpdateResource('{{route('eventCategories.store')}}','{{route('eventCategories.index')}}','post','add-form')"> Add Category</button>
                        </div>
                    </form>
                </div>
            </div>
...
@section('extra-script')
    ...
    <script>
        ...
        $('.editCatBtn').on('click', function() {
            ...
            $.ajax({
                type: 'get',
                url: url + '/admin/eventCategories/' + catID + '/edit',
