...
<section class="main_padding my-5">
    ...
    <form action="{{route('stripe.post')}}" id="create-order" class="my-3 require-validation needs-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ $admin->stripe_public_key }}" method="post" novalidate>
        ...
        <input type="hidden" name="specialist_id" value="{{ $booking->seller_id }}">
        <input type="hidden" name="id" value="{{ $booking->id }}">
        <input type="hidden" name="payment_for" value="appointment">
        <input type="hidden" name="amount" value="{{ $booking->service_cost}}">
            ...
            <div class="col-lg-4 p_Sm-0">
                <div class="bg-f8f8f8 px-2 py-3">
                    <div class="d-flex w-100">
                        <div><img src="{{ asset($booking->specialist->picture)}}" style="border-radius:50%;width:80px; height:80px" alt=""></div>
                        <div class="pl-3 w-100">
                            <div class="h5 m-0 robotoMedium">{{$booking->service_name}}</div>
                            <div class="f-18 m-0 robotoRegular"><span class="cl-3ac754 ">By:</span> {{$booking->specialist->username}}</div>
                            <div class="d-flex pt-3 justify-content-between">
                                @php
                                    $day = date('d',($booking->service_date)/1000);
                                    if($day=='1' || $day=='21' || $day=='31'){$sup = 'st'; }
                                    else if($day=='2' || $day=='22'){$sup = 'nd'; }
                                    else if($day=='3' || $day=='23'){$sup = 'rd'; }
                                    else{$sup = 'th'; }
                                @endphp
                                <div class=" m-0 robotoLight">Date : {{ date('M',($booking->service_date)/1000) }} {{ $day }}<sup>{{$sup}}</sup> {{ date('Y',($booking->service_date)/1000) }}</div>
                                <div class=" m-0 robotoLight">Time : {{ date('h:i a',($booking->service_date)/1000) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="border-bottom pb-3">
                    <div class="d-flex pt-3 pb-2">
                        ...
                        <div class="col-lg-4 p-0 text-right f-18 robotoRegular">${{$booking->service_cost}}</div>
                    </div>
                    <div class="d-flex py-2">
                        <div class="col-lg-8 p-0 f-18 cl-a9a9a9 robotoRegular ">Specialist Rate</div>
                        <div class="col-lg-4 p-0 text-right f-18 cl-a9a9a9 robotoRegular">${{$booking->service_cost - ($booking->service_cost*serviceFee())/100}}</div>
                    </div>
                    <div class="d-flex">
                        <div class="col-lg-8 p-0 f-18 cl-a9a9a9 robotoRegular ">Service Fee</div>
                        <div class="col-lg-4 p-0 text-right f-18 cl-a9a9a9 robotoRegular">${{($booking->service_cost*serviceFee())/100}}</div>
                    </div>
                </div>
...
@section('extra-script')
    ...
    <script>
        ...
        setTimeout(() => {
            ...
            @if(Session::has('success'))
                swal("Payment Successful!", "{{Session::get('success')}}", "success").then((value) => { if(value){window.location = '{{route("appointments.index")}}';} });
            @endif
