...
@section('content')
    ...
    <section class="main_padding my-5">
        ...
        <form action="{{route('stripe.payment.for.event.ticket')}}" id="create-order" class="my-3 require-validation needs-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ $admin->stripe_public_key }}" method="post" novalidate>
            @csrf
            <input type="hidden" name="id" value="{{$event->id}}">
            <input type="hidden" name="payable_amount" value="{{$event->price}}">
                ...
                <div class="col-lg-4 p_Sm-0">
                    <div class="bg-f8f8f8 px-2 py-3">
                        <div class="d-flex w-100">
                            <div><img src="{{ asset($event->user->picture)}}" style="border-radius:50%;width:80px; height:80px" alt=""></div>
                            <div class="pl-3 w-100">
                                <div class="h5 m-0 robotoMedium">{{$event->category->title}}</div>
                                <div class="f-18 m-0 robotoRegular"><span class="cl-3ac754 ">By:</span> {{$event->user->username}}
                                </div>
                                <div class="d-flex pt-3 justify-content-between">
                                    @php
                                        $day = date('d',strtotime($event->date_time));
                                        if($day=='1' || $day=='21' || $day=='31'){$sup = 'st'; }
                                        else if($day=='2' || $day=='22'){$sup = 'nd'; }
                                        else if($day=='3' || $day=='23'){$sup = 'rd'; }
                                        else{$sup = 'th'; }
                                    @endphp
                                    <div class=" m-0 robotoLight">Date : {{ date('M',strtotime($event->date_time)) }} {{ $day }}<sup>{{$sup}}</sup> {{ date('Y',strtotime($event->date_time)) }}</div>
                                    <div class=" m-0 robotoLight">Time : {{ date('h:i a',strtotime($event->date_time)) }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="border-bottom pb-3">
                        <div class="d-flex pt-3 pb-2">
                            ...
                            <div class="col-lg-4 p-0 text-right f-18 robotoRegular">${{$event->price}}</div>
                        </div>
                        <div class="d-flex py-2">
                            <div class="col-lg-8 p-0 f-18 cl-a9a9a9 robotoRegular ">Specialist Rate</div>
                            <div class="col-lg-4 p-0 text-right f-18 cl-a9a9a9 robotoRegular">${{$event->price}}</div>
                        </div>
                        <!-- <div class="d-flex">
                            <div class="col-lg-8 p-0 f-18 cl-a9a9a9 robotoRegular ">Service Fee</div>
                            <div class="col-lg-4 p-0 text-right f-18 cl-a9a9a9 robotoRegular">${{($event->price*20)/100}}</div>
                        </div> -->
                    </div>
    ...
    <script>
        ...
        const paymentRadio=(e, sectionName)=>{
            ...
            // console.log('chal rhaa');
                $('#create-order').attr('action',"{{route('stripe.payment.for.event.ticket')}}");
            ...
            if(sectionName == 'paypal'){
                $('#create-order').attr('action',"{{route('paypal.payment.for.event.ticket')}}")
                // console.log('chal rhaa');
        ...
        setTimeout(() => {
            ...
            @if(Session::has('success'))
                ...
                var err = "";
                err += "<p class='pb-0 mb-2'>Note: You may enter the event at least 10 minutes before start time</p>";
                err += "<p class='text-left pb-0 mb-2'>To Enter Your Purchased Event</p>";
                err += "<p class='text-left pb-0 mb-1 ml-2'>Please go to:</p>";
                err += "<p class='text-left pb-0 mb-1 ml-3'><img class='mr-1' src='{{ asset('assets/frontend/images/tick.png') }}' style='width:18px;'/>Profile</p>";
                err += "<p class='text-left pb-0 mb-1 ml-3'><img class='mr-1' src='{{ asset('assets/frontend/images/tick.png') }}' style='width:18px;'/>Event Purchases</p>";
                wrapper.innerHTML = err;
                swal({
                    icon: "success",
                    text: "{{ __('Payment Successful!') }}",
                    content: wrapper,
                    type: "success",
                }).then((value) => {if(value){window.location = '{{route("getEventTicketForSpecificUser")}}';} });
            @endif

            @if(Session::has('error'))
                swal("Payment Failed!", "{{Session::get('error')}}", "success").then((value) => {if(value){ window.location = '';}});
            @endif
