...
<head>
    ...
    .loader{
        ...
        .video-stream-div{width: 55%; }
        .chat-div{ width:45%;}
        
        @media screen and (max-width: 768px) {
            .video-chat-div{flex-direction:column;}
            .video-div,.chat-div,.video-stream-div{width: 100% !important;}
...
<body style="display:block !important;">
    <div class="d-none checkClass" data-type="" data-name="" data-check="false"></div>
    ...
    @yield('navbar')
        <section class="main_padding pt-70 px-50 padding_dashboard">
            <div class="row m-0 justify-content-center">
                <div class="col-md-3 col-lg-3 col-sm-12 p-0 box_shadow1 borderRadius-12px pt-4 pb-5 profile-Sidebar">
                    <p class="border-bottom text-center f-21 cl-616161">Your Profile</p>
                    <div class="d-flex align-items-center flex-column">
                        <div class="dashboard_id w-100 text-center">
                    ...
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        ...
                        @if (Auth::user()->type == 'seller')
                            ...
                            <a class="nav-link cl-000000 {{ Request::is('posts') ? 'active' : ''  }}"  href="{{ url('/posts') }}" >Create a Blog Post</a>
                            <a class="nav-link cl-000000 {{ Request::is('user/events') ? 'active' : ''  }}"  href="{{ url('/user/events') }}" >Create an Event</a>
                        @endif
                        ...
                        @if(Auth::user()->type == 'buyer')<a class="nav-link cl-000000 {{ Request::is('eventTickets') ? 'active' : ''  }}" href="{{ url('eventTickets') }}">Event Purchases</a>@endif
                        <a class="nav-link cl-000000 {{ Request::is('change-password') ? 'active' : ''  }}" href="{{ url('change-password') }}">Password</a>
                    </div>
                </div>

                <div class="col-md-8 col-lg-8 col-sm-12 pt-4 p-0 ml-4 perso_Tab box_shadow1 borderRadius-12px">
                    @yield('content') 
                </div>
            </div>
        </section>
    ...
    <section class="main_padding pt-70  w-100" style="margin-top: 30px;">
        ...
        <div class="modal-dialog modal-min-max-width" role="document">
            <div class="modal-content">
                    ...
                    <div class="embed-responsive embed-responsive-21by9">
                        ...
                        <html lang="en">
                            ...
                            <body class="agora-theme">
                                <div class="row video-chat-div" style="width:100% !important; overflow:hidden;">
                                    <div class="video-stream-div">
                                        <form id="form" class="row l12 s12 " >
                                            <div class="row container col l12 s12 p-0">
                                            ...
                                            <div class="card-footer border-0 pl-3 pr-3" >
                                                ...
                                                <form id="chat-form" method="post" class="mt-1 ">
                                                    <div class="input-group border-top d-flex align-items-center pb-2">
                                                        <textarea  id="video-message-content" oninput="onTextInput(this)"  name="content" class="form-control  pl-0" placeholder="Type your message ..." autocomplete="off"></textarea>
                                ...
                                <style>
                                    #local_stream > :first-child{height:30% !important;width:30% !important;left:67% !important; top:70% !important;}
                                </style>
                            </body>
                        </html>
                    </div>
                ...
                <div class="modal-footer border-0">
                    ...
                    @endif
                    <button type="button" class="btn btn-secondary bg-3ac574 full-screen-btn" onclick="fullScreen(this)">Full Screen</button>
                    {{-- <button type="button" class="btn btn-secondary bg-3ac574 end-call" data-dismiss="modal" onclick="endCall()">End Call</button> --}}
                    <button type="button" class="btn btn-secondary bg-3ac574 end-client-call d-none" @if(Auth::check()) @if(Auth::user()->type=='seller')data-dismiss="modal"@endif @endif onclick="@if(Auth::check()) @if(Auth::user()->type=='seller') endCall() @else clientEndCall() @endif @endif">End Call</button>
                    <button type="button" class="btn btn-secondary bg-3ac574 end-message-call d-none" @if(Auth::check()) @if(Auth::user()->type=='seller')data-dismiss="modal"@endif @endif onclick="endCall()">End Call</button>
                    <button type="button" class="btn btn-secondary bg-3ac574 end-call-check d-none" data-dismiss="modal"></button>
                </div>
            </div>
        </div>
    </div>
...
<!-- T E N    S E C T I O N  E N D  -->
    ...
    {{-- <script>
        $(document).ready(function(){
            $.getScript("{{ asset('assets/vendor/sweetalert/sweetalert.min.js') }}");
            $.getScript("{{ asset('assets/admin/plugins/select2/js/select2.full.min.js') }}");
            $.getScript("{{ asset('assets/admin/plugins/datatables/jquery.dataTables.min.js')}}");
            $.getScript("{{ asset('assets/admin/plugins/datatables-bs4/js/dataTables.bootstrap4.min.js')}}");
            $.getScript("{{ asset('assets/admin/plugins/datatables-responsive/js/dataTables.responsive.min.js')}}");
            $.getScript("{{ asset('assets/admin/plugins/datatables-responsive/js/responsive.bootstrap4.min.js')}}");
            $.getScript("{{ asset('assets/admin/plugins/datatables-buttons/js/dataTables.buttons.min.js')}}");
            $.getScript("{{ asset('assets/admin/plugins/datatables-buttons/js/buttons.bootstrap4.min.js')}}");
        });
    </script> --}}
    ...
    <script>
        ...
        function focusOnInput()
        {
            ...
            firebase.database().ref('list').child('u'+sender).once("value", function(ysnapshot) {
                $.each(ysnapshot.val(),function(i,v){
                    console.log(i);
                    if(v.message){content = v.message;}else{content ='';}
                    firebase.database().ref('/list/u'+sender+'/'+i).set({id:v.id,message:v.message,sender_id:v.sender_id,sender_name:v.sender_name,sender_photo:v.sender_photo,time:v.time,unread:1})   
                });
            });
        }
        ...
        function bookingReject(id){
            swal({
                text: 'Are you sure you want to continue? Your profile and data will be removed from system',
                icon: "warning",
                buttons: {
                    cancel: "Cancel",
                    confirm: "OK"
        ...
        @if(Auth::check())
            ...
            function makeCall(e){
                ...
                if(typeof $(e).data('booking-id')!='undefined' && $(e).data('booking-id')!=''){
                    $('.end-client-call').removeClass('d-none');
                    $('.end-message-call').addClass('d-none');
                }
                else if($(e).data('booking-id')==''){
                    ...
                    if(snapshot.exists()){
                        $.each(snapshot.val(),function(){
                            ...
                            chat_element +='<div class="d-flex mt-4">';
                                chat_element +='<div class="col-lg-1 col-md-1 col-sm-1 col-1 p-0"><div class="parent"><img src="'+this.photo+'" class="rounded-circle img-fluid smallProfile" alt="" srcset=""></div></div>';
                ...
                $('#review_item_id').val($(e).data('booking-id'));
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $(".checkClass").attr('data-type','appointment');
            ...
            function makeEventCall(id,name){
                $('#single-event-channel-name').val(name);
                $('.video-title').remove();
                $('.end-client-call').addClass('d-none');
                $('.end-message-call').removeClass('d-none');
                firebase.database().ref('event').child(name).on('value',(snapshot) => {
                    chat_element = '';
                    if(snapshot.exists()){
                        $.each(snapshot.val(),function(){
                            var mt = "mt-0";
                            // moment(this.time).tz('{{ Auth::user()->timezone }}').format('M-D-Y h:mmA')
                            chat_element +='<div class="d-flex mt-4">';
                                chat_element +='<div class="col-lg-1 col-md-1 col-sm-1 col-1 p-0"><div class="parent"><img onerror="this.onerror=null;this.src='+defaultUserPicture+'" src="'+this.photo+'" class="rounded-circle img-fluid smallProfile" alt="" srcset=""></div></div>';
                                chat_element +='<div class="col-lg-11 pl-3">';
                                    chat_element +='<div>'+this.name[0].toUpperCase() + this.name.slice(1)+'<span class="f-12 pl-2">'+moment(this.time,"x").tz('{{ Auth::user()->timezone }}').format('M-D-Y h:mm a')+'</span></div> ';
            ...
            function messageSend() {
                ...
                if(img!=''){
                    $.ajax({
                        ...
                        data: formData,
                        success:function(data){
                            firebase.database().ref('message').child(rsender_eciever).get().then((snapshot) => {
            ...
            if($('#single-event-channel-name').val()!=''){
                firebase.database().ref('event').child($('#single-event-channel-name').val()).on('value',(snapshot) => {
                    chat_element = '';
                    if(snapshot.exists()){
                        $.each(snapshot.val(),function(){
                            var mt = "mt-0";
                        ...
                        data.map(v=>{
                            ...
                            if(typeof(element) == 'object' && element == null){
                                html += '<a class="dropdown-item d-flex row m-0 pt-2"  id="nAppointment'+v.id+'" href="'+v.url+'">';
                                    ...
                                    html+='</div>';
                                    html+='<div class="col-md-6 pl-2 pt-1 p-0">';
